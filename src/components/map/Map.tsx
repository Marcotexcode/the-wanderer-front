...
function Map() {
  ...
  useEffect(() => {
    ...
    const fetchPlayerHomeAndStructures = async () => {
      try {
        ...
        const response = await axios.get('http://localhost:3001/get-player-home-and-structure', {
          headers: {
  ...
  const createSvgElement = useCallback((type: string, attributes: Record<string, any>) => {
  ...
  const createLocationElements = useCallback((citiesGroup: SVGGElement, location: { id: string; x: number; y: number; fontSize: number; name: string; }) => {
  ...
  const createPlayerHomeElements = useCallback((citiesGroup: SVGGElement, playerHome: { id: string; x: number; y: number; name: string; }) => {
    ...
    const text = createSvgElement('text', {
      ...
      onClick: (e: Event) => handleLocationClick(e, { ...playerHome, type: 'playerHome' })
    });
    citiesGroup.appendChild(text);
  ...
  useEffect(() => {
    if (mapRef.current) {
      ...
      if (citiesGroup instanceof SVGGElement) {
        locations.forEach(location => createLocationElements(citiesGroup, location));
  ...
  const renderPlayerHomeContent = () => (
    <div className="modal-structures">
      ...
      <div className="structure-cards">
        {selectedLocation.structureIds && selectedLocation.structureIds.length > 0 ? (
